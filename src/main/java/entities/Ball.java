 ...
 *
 * <p>Physikalisches Modell pro Frame:
 *
 * <ol>
 *   <li>Schwerkraft und Luftreibung auf den Geschwindigkeitsvektor anwenden
 *   <li>Geschwindigkeit auf {@code MAX_SPEED} begrenzen
 *   <li>Position aktualisieren
 *   <li>Wand-, Latten- und Spielerkollisionen auflösen
 * </ol>
 *
 * Die Spieler-Kollision verwendet zwei Kreise (Kopf und Körper/Beine), damit der Ball nicht durch
 * die Beine hindurchfliegt.
 */
public class Ball {
  ...
  /** Letzter Spieler, der den Ball berührt hat. Wird für die Eigentor-Erkennung ausgewertet. */
  ...
  /** Radius des Balls in Spielkoordinaten. */
  ...
  /**
   * Zeichnet den Bodenschatten des Balls. Muss <em>vor</em> den Spielern aufgerufen werden, damit
   * der Schatten unter ihnen liegt.
   *
  ...
  /**
   * Zeichnet den Ball mit Rotation. Der Schatten wird separat über {@link #drawShadow} gerendert.
   *
   * @param g Grafik-Kontext
   */
  public void draw(Graphics2D g) {
    ...
    g.fillOval(-RADIUS, -RADIUS, SIZE, SIZE);

    g.setColor(Color.BLACK);
    g.fillPolygon(new int[] {-5, 5, 10, 0, -10}, new int[] {-10, -10, 0, 10, 0}, 5);

    g.setStroke(new BasicStroke(2));
    g.drawOval(-RADIUS, -RADIUS, SIZE, SIZE);

    g.setTransform(old);
  }

  /**
   * Begrenzt die Gesamtgeschwindigkeit auf {@code MAX_SPEED}. Verhindert exponentielles Anwachsen
   * bei Mehrfachkollisionen (Einklemm-Situationen).
   */
  ...
  /**
   * Berechnet die Kollision zwischen Ball und Spieler. Geprüft werden zwei Kreise: zuerst der Kopf,
   * dann Körper/Beine – so kann der Ball nicht mehr durch die Beine hindurchfliegen. Pro Frame wird
   * höchstens eine der beiden Kollisionen aufgelöst.
   *
  ...
  /**
   * Löst eine Kreis-Kollision mit einem Körperteil des Spielers auf. Verwendet distanzbasierte
   * Überlappungskorrektur, um den Tunneling-Effekt zu verhindern.
   *
  ...
  /**
   * Prüft Kollisionen mit Wänden, Boden und Decke und löst sie auf. Bei Bodenkontakt wird die
   * Drehgeschwindigkeit aus {@code vx} abgeleitet, damit der Ball sichtbar rollt statt zu gleiten.
   *
  ...
  private void checkWallCollisions(Stadium stadium) {
    int limitL = stadium.leftGoal.backX + RADIUS;
    int limitR = stadium.rightGoal.backX - RADIUS;
    ...
    if (y > groundY - RADIUS) {
      ...
      spinVelocity = Math.toDegrees(vx / (double) RADIUS);
    }
  }
  ...
  private void checkCrossbar(Stadium stadium) {
    ...
    if (y > crossY - RADIUS && y < crossY + RADIUS + 15) {
      if (x > stadium.leftGoal.backX && x < stadium.leftGoal.frontX + RADIUS) {
        resolveCrossbarHit(stadium.leftGoal, crossY);
      } else if (x > stadium.rightGoal.frontX - RADIUS && x < stadium.rightGoal.backX) {
  ...
  /**
   * Löst einen Lattentreffer auf: kehrt die Vertikalgeschwindigkeit um, korrigiert die Position und
   * löst den Wackeleffekt aus.
   *
  ...
  private void resolveCrossbarHit(Goal goal, int crossY) {
    ...
    vy = -vy * 0.7;
    y = hitFromTop ? crossY - RADIUS - 2 : crossY + RADIUS + 12;
