 ...
 *
 * <p>Enthält drei Karten: Hauptmenü ({@code "MENU"}), Steuerung ({@code "CONTROLS"}) und
 * Team-Auswahl ({@code "TEAM_SELECT"}). Der Wechsel zwischen ihnen erfolgt ohne neues Fenster. Der
 * Animations-Timer läuft nur, solange das Panel angezeigt wird ({@link #addNotify} / {@link
 * #removeNotify}), damit geschlossene Menü-Fenster keine Timer hinterlassen.
 */
public final class MenuPanel extends JPanel {
    ...
    menuContent =
        new JPanel(new GridBagLayout()) {
          ...
          protected void paintComponent(Graphics g) {
            ...
            GradientPaint bg =
                new GradientPaint(0, 0, new Color(4, 6, 22), 0, getHeight(), new Color(8, 20, 55));
            g2.setPaint(bg);
  ...
  /**
   * Stoppt den Animations-Timer, wenn das Panel aus der Hierarchie entfernt wird (z.&nbsp;B. beim
   * Schließen des Menü-Fensters) – verhindert Timer-Leaks.
   */
  @Override
  ...
  private Polygon createStarburst(int cx, int cy, int outerR, int innerR, int spikes) {
    ...
    for (int i = 0; i < spikes * 2; i++) {
      ...
      p.addPoint(cx + (int) (Math.cos(angle) * r), cy + (int) (Math.sin(angle) * r));
    }
    return p;
  }
    ...
    JButton btn =
        new JButton(text) {
          ...
          protected void paintComponent(Graphics g) {
            ...
            g2.setColor(
                new Color(
                    accent.getRed() / 6, accent.getGreen() / 6, accent.getBlue() / 6 + 8, 220));
