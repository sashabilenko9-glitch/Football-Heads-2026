 ...
 *
 * <p>Entwurfsentscheidungen:
 *
 * <ul>
 *   <li>{@link #p1Active} / {@link #p2Active} verfolgen ausschließlich die Maus (für {@code
 *       setP1MobileInput} / {@code setP2MobileInput}).
 *   <li>{@link #paintComponent} liest den visuellen Zustand aus dem {@link InputManager} (Tastatur
 *       ODER Maus), sodass beide Eingabequellen angezeigt werden.
 *   <li>Ein eigener 16-ms-Timer zeichnet das Panel unabhängig vom {@code GamePanel} neu. Er läuft
 *       nur, solange das Panel angezeigt wird ({@link #addNotify} / {@link #removeNotify}), damit
 *       geschlossene Spielfenster keine Timer hinterlassen.
 *   <li>Während Pause und Endbildschirm verschwinden die Tasten und lassen Mausklicks durch ({@link
 *       #contains} liefert {@code false}).
 * </ul>
 *
 * Hinweis: Da nur ein Mauszeiger existiert, kann per Maus immer nur eine Taste gleichzeitig
 * gehalten werden – die Panels dienen primär als Anzeige und Gelegenheits-Steuerung, die Tastatur
 * bleibt die Hauptsteuerung.
 */
public final class MobileControlsPanel extends JPanel {
  ...
  private final transient InputManager inputManager;
  private final Timer repaintTimer;

  /**
   * Erstellt das Overlay und registriert die Maus-Listener. Der Repaint-Timer wird erst in {@link
   * #addNotify} gestartet.
   *
  ...
  /**
   * Stoppt den Repaint-Timer, wenn das Panel aus der Hierarchie entfernt wird (z.&nbsp;B. beim
   * Schließen des Spielfensters) – verhindert Timer-Leaks.
   */
  @Override
  ...
  /**
   * @return {@code true}, wenn die Tasten aktuell ausgeblendet sind (Pause oder Endbildschirm)
   */
  ...
  /**
   * Beschränkt die Treffer-Fläche des Panels auf die Tastenbereiche, damit Mausklicks außerhalb der
   * Tasten an das Spielfeld durchgereicht werden. Im ausgeblendeten Zustand gehen alle Klicks
   * durch.
   *
  ...
  /**
   * Liefert das Rechteck einer Bildschirm-Taste. Index-Belegung: {@code 0} = springen, {@code 1} =
   * links, {@code 2} = schießen, {@code 3} = rechts.
   *
  ...
  /**
   * Liefert die um 15 Pixel vergrößerte Treffer-Fläche einer Taste, um die Bedienung mit der Maus
   * zu erleichtern.
   *
   * @param leftCluster {@code true} für den linken Cluster (Spieler 1), sonst rechter Cluster
   * @param idx Tastenindex (0–3)
  ...
  /**
   * Wertet ein Maus-Ereignis aus und überträgt den neuen Tastenzustand an das Spielpanel. Auch das
   * Loslassen wird immer weitergereicht, damit keine Eingabe hängen bleibt.
   *
  ...
  /**
   * Zeichnet beide Tastencluster. Der Gedrückt-Zustand wird aus dem {@link InputManager} gelesen,
   * sodass Tastatur- und Mauseingaben gleichermaßen sichtbar sind. Während Pause und Endbildschirm
  ...
  /**
   * Zeichnet eine einzelne Bildschirm-Taste im Glas-Stil. Im gedrückten Zustand „sinkt" die Taste
   * nach unten und leuchtet cyan.
   *
   * @param g Grafik-Kontext
  ...
  private void drawKey(Graphics2D g, Rectangle r, String label, boolean pressed) {
    ...
    // Vorderseite
    g.setColor(pressed ? new Color(0, 180, 255, 140) : new Color(0, 0, 0, 90));
    g.fillRoundRect(r.x, r.y + lift, r.width, r.height, arc, arc);

    // Rahmen
    g.setStroke(new BasicStroke(pressed ? 2.5f : 1.8f));
    g.setColor(pressed ? new Color(0, 220, 255, 255) : new Color(255, 255, 255, 170));
