...
/**
 * Schmale Button-Leiste oben rechts im Spielfenster.
 * Bietet Schnellzugriff auf Neustart, Hauptmenü und das Ein-/Ausblenden
 * der Bildschirm-Tasten während des laufenden Spiels.
 */
public class InGameUI extends JPanel {
     ...
     * @param controlsOverlay Bildschirm-Tasten-Overlay (für den TASTEN-Umschalter);
     *                        {@code null} blendet den Umschalter aus
     */
    public InGameUI(GamePanel gamePanel, GameFrame frame, MobileControlsPanel controlsOverlay) {
        ...
        if (controlsOverlay != null) {
            add(createNeonButton("TASTEN", new Color(0, 110, 170),
                    e -> controlsOverlay.setVisible(!controlsOverlay.isVisible())));
        }

        add(createNeonButton("RESTART", new Color(0, 160, 255),
                e -> gamePanel.restartGame()));

        add(createNeonButton("MENÜ", new Color(180, 30, 70),
                e -> frame.returnToMenu()));
    }
