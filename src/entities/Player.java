 ...
 * <p>
 * Steuerung über boolesche Flags ({@link #left}, {@link #right}, {@link #jumping}, {@link #kicking}),
 * die vom {@code GamePanel} aus dem {@code InputManager} gesetzt werden.
 * Animationen (Arme, Beine, Haare) werden mathematisch aus der aktuellen Geschwindigkeit abgeleitet.
 * <p>
 * Koordinatenkonvention: {@link #y} ist der Fußpunkt. Der Sprite erstreckt sich von
 * {@code y - 150} (Kopfoberkante) bis {@code y} (Sohlen), sodass die Füße exakt auf
 * {@link Field#GROUND_Y} stehen – auf derselben Linie, auf der auch der Ball aufliegt.
 */
public class Player {
    ...
    public void update(Ball ball, Stadium stadium, Player opponent) {
        ...
        if (jumping && onGround) { vy = -16.0; onBar = false; }
        vy += Field.GRAVITY;
        ...
        if (y > Field.GROUND_Y) { y = Field.GROUND_Y; vy = 0; }
        x = Math.max(stadium.leftGoal.backX + 15, Math.min(stadium.rightGoal.backX - 15, x));

        if (Math.abs(vx) > 0.5) walkCycle += 0.25; else walkCycle = 0;
        updateHairPhysics();
    ...
    /**
     * Zeichnet den Spieler (Schatten, Beine, Körper, Kopf, Arme).
     * Der Fußpunkt {@link #y} entspricht den Sohlen.
     *
    ...
    /**
     * Verhindert, dass Spieler ineinander laufen (Überlappungskorrektur mit Energieaustausch).
     * Wird nur von Spieler 1 ausgeführt, damit dieselbe Überlappung nicht zweimal
     * pro Frame aufgelöst wird. Nach der Korrektur werden beide Spieler wieder auf
     * Boden und Spielfeldgrenzen geklemmt, damit niemand im Boden oder Tor versinkt.
     *
    ...
    /**
     * Kollisionen mit der Torlatte: Landen und Stehen auf der Latte,
     * Kopfstoß von unten sowie seitliches Blocken auf Lattenhöhe.
     *
    ...
    private void checkGoalCollision(Goal goal) {
        ...
        boolean inBarX = x + playerRadius > crossbar.x
                      && x - playerRadius < crossbar.x + crossbar.width;
        ...
        if (headTop < crossbar.y + crossbar.height && headBottom > crossbar.y) {
            if (x + playerRadius > crossbar.x && x < crossbar.x + crossbar.width / 2 && vx > 0) {
                ...
                goal.startShake();
                    ...
                    && x > crossbar.x + crossbar.width / 2 && vx < 0) {
    ...
    /**
     * Zeichnet einen Arm (vordere oder hintere Ebene).
     * Die Arme schwingen gegenläufig zueinander (Phasenversatz {@code π}).
     *
     * @param g            Grafik-Kontext
    ...
    /**
     * Zeichnet den Kopf. Wenn {@link #headImage} gesetzt ist, wird es gerendert;
     * andernfalls wird der prozedurale Standard-Kopf gezeichnet.
     * Die Pupillen verfolgen den Ball per {@code atan2}-Winkel.
     *
     * @param g    Grafik-Kontext
     * @param ix   X-Mittelpunkt des Spielers
     * @param iy   Y-Fußpunkt des Spielers
    ...
    private void drawHead(Graphics2D g, int ix, int iy, Ball ball) {
        int headSize = 76;
        int drawX    = ix - headSize / 2;
        int drawY    = iy - 150;

        if (headImage != null) {
            if (facingRight) {
                g.drawImage(headImage, drawX, drawY, headSize, headSize, null);
            } else {
                g.drawImage(headImage, drawX + headSize, drawY, -headSize, headSize, null);
            }
            return;
        }
    ...
    /**
     * Zeichnet die Beine mit optionaler Tritt-Verschiebung.
     * Die Sohlen enden exakt auf dem Fußpunkt {@code iy}.
     *
     * @param g  Grafik-Kontext
     * @param ix X-Mittelpunkt des Spielers
     * @param iy Y-Fußpunkt des Spielers
